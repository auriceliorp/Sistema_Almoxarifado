...
# Importe apenas os modelos que estão funcionando com certeza
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        # Verificação segura para current_user e is_admin
...
def login():
    ...
    if request.method == "POST":
        ...
        if usuario and hasattr(usuario, 'senha') and check_password_hash(usuario.senha, senha):
            login_user(usuario)
            return redirect(url_for("main.index"))
        else:
...
# --- Rotas para Naturezas de Despesa (MANTIDAS COMO ESTAVAM) ---
...
def listar_naturezas_despesa():
    try:
        ...
        flash("Erro ao carregar Naturezas de Despesa.", "danger")
        return redirect(url_for("main.index"))
...
def cadastrar_natureza_despesa():
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            print(f"Erro ao adicionar natureza: {e}")
...
def editar_natureza_despesa(id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            print(f"Erro ao editar natureza: {e}")
...
def excluir_natureza_despesa(id):
    ...
    try:
        ...
        db.session.rollback()
        print(f"Erro ao excluir natureza: {e}")
        flash(f"Erro ao excluir Natureza de Despesa: {e}", "danger")

    return redirect(url_for("main.listar_naturezas_despesa"))

# --- Rotas para Itens (MANTIDAS COMO ESTAVAM) ---
@main_bp.route("/itens")
@login_required
def listar_itens():
    try:
        ...
        flash("Erro ao carregar Itens.", "danger")
        return redirect(url_for("main.index"))
...
def cadastrar_item():
    ...
    if request.method == "POST":
        ...
        novo_item = Item(
            ...
            saldo_atual=0 # Inicializa saldo_atual
        )
...
# --- Placeholder para Movimentações (para evitar erro 404 se linkado) ---
@main_bp.route("/movimentos")
@login_required
def listar_movimentos_placeholder():
    flash("Funcionalidade de Movimentações em desenvolvimento.", "info")
    return redirect(url_for("main.index"))
