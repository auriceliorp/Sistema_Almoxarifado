...
from flask import Blueprint, render_template, request, redirect, url_for, flash from flask_login import login_required, current_user from werkzeug.utils import secure_filename from extensoes import db from models import BemPatrimonial, Usuario, Grupo from PIL import Image  # Pillow para redimensionar imagem import os

Cria o blueprint do módulo de Patrimônio

patrimonio_bp = Blueprint('patrimonio_bp', name, url_prefix='/patrimonio')

Pasta para salvar as fotos dos bens

UPLOAD_FOLDER = 'static/fotos_bens' ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif'}
...
def arquivo_permitido(filename): return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

------------------------ ROTA: LISTAGEM DE BENS ------------------------

@patrimonio_bp.route('/bens') @login_required def listar_bens(): termo = request.args.get('termo') query = BemPatrimonial.query.filter_by(excluido=False)
...
@patrimonio_bp.route('/bens/novo', methods=['GET', 'POST']) @login_required def novo_bem(): usuarios = Usuario.query.order_by(Usuario.nome).all() grupos = Grupo.query.order_by(Grupo.nome).all()
...
@patrimonio_bp.route('/bens/editar/int:id', methods=['GET', 'POST']) @login_required def editar_bem(id): bem = BemPatrimonial.query.get_or_404(id) usuarios = Usuario.query.order_by(Usuario.nome).all() grupos = Grupo.query.order_by(Grupo.nome).all()
...
@patrimonio_bp.route('/bens/visualizar/int:id') @login_required def visualizar_bem(id): bem = BemPatrimonial.query.get_or_404(id) return render_template('patrimonio/visualizar_bem.html', bem=bem, usuario=current_user)
