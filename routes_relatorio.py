...
from flask import Blueprint, render_template, request from flask_login import login_required from sqlalchemy import extract, func from app_render import db from models import NaturezaDespesa, Grupo, Item, EntradaItem, SaidaItem from datetime import datetime from decimal import Decimal
...
@relatorio_bp.route('/relatorio/mapa_fechamento') @login_required def mapa_fechamento(): # Obtem mês e ano dos parâmetros da URL ou usa o mês atual mes = request.args.get('mes', datetime.now().month, type=int) ano = request.args.get('ano', datetime.now().year, type=int)
