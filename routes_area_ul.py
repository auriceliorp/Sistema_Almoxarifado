...
@area_ul_bp.route('/areas') @login_required def lista_areas(): areas = Local.query.order_by(Local.descricao).all() if is_ajax(): return render_template('partials/area/lista_area.html', areas=areas) return render_template('organizacao/dashboard_organizacao.html')

@area_ul_bp.route('/areas/nova', methods=['GET', 'POST']) @login_required def nova_area(): if request.method == 'POST': descricao = request.form.get('descricao') if not descricao: flash('Descrição é obrigatória.') if is_ajax(): return render_template('partials/area/form_area.html', area=None) return redirect(url_for('area_ul_bp.nova_area'))
...
@area_ul_bp.route('/areas/editar/int:id', methods=['GET', 'POST']) @login_required def editar_area(id): area = Local.query.get_or_404(id) if request.method == 'POST': descricao = request.form.get('descricao') if not descricao: flash('Descrição é obrigatória.') if is_ajax(): return render_template('partials/area/form_area.html', area=area) return redirect(url_for('area_ul_bp.editar_area', id=id))
...
@area_ul_bp.route('/areas/excluir/int:id', methods=['POST']) @login_required def excluir_area(id): area = Local.query.get_or_404(id) db.session.delete(area) db.session.commit() flash('Área excluída com sucesso!')
...
@area_ul_bp.route('/') @login_required def lista_ul(): uls = UnidadeLocal.query.order_by(UnidadeLocal.codigo).all() if is_ajax(): return render_template('partials/ul/lista_ul.html', uls=uls) return render_template('organizacao/dashboard_organizacao.html')
...
@area_ul_bp.route('/excluir/int:id', methods=['POST']) @login_required def excluir_ul(id): ul = UnidadeLocal.query.get_or_404(id) db.session.delete(ul) db.session.commit() flash('Unidade Local excluída com sucesso!')
